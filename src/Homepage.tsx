...
const Homepage = () => {
  ...
  const [allEmployeesData, setAllEmployeesData] = useState<Employee[] | null>(null);
  ...
  const cardDeleteHandler = () => {
    const filteredData = allEmployeesData?.filter(employee => !selectedEmployeeIds.has(employee.id)) ?? null;
    setAllEmployeesData(filteredData);
    setSelectedEmployeeIds(new Set());
  };

  const individualDeleteHandler = (id: string) => {
    const filteredData = allEmployeesData?.filter(employee => employee.id !== id) ?? null;
    setAllEmployeesData(filteredData);
  };
        ...
        <div className='flex flex-wrap gap-6 mt-36 justify-center items-center'>
          {searchedData === null && searchText.length === 0 ? (
            ...
            <div className='w-full h-full justify-center items-center'>
              <NoResultsFound setSearchedData={setSearchedData} setSearchText={setSearchText}/>
            </div>
          )}
        </div>
      ) : (
